...
pub enum KeyframesStepValue {
    ...
    Declarations(Arc<StyleRefCell<PropertyDeclarationBlock>>),
    ComputedValues,
}
    ...
    fn new(percentage: KeyframePercentage,
        ...
        let declared_timing_function = match value {
            KeyframesStepValue::Declarations(ref block) => {
                block.borrow_read_only(token).declarations.iter().any(|&(ref prop_decl, _)| {
                    match *prop_decl {
...
#[allow(unsafe_code)]
fn get_animated_properties(keyframe: &Keyframe, token: &ReadOnlyToken) -> Vec<TransitionProperty> {
    let mut ret = vec![];
    // NB: declarations are already deduplicated, so we don't have to check for
    // it here.
    for &(ref declaration, _) in keyframe.block.borrow_read_only(token).declarations.iter() {
...
impl KeyframesAnimation {
    pub fn from_keyframes(keyframes: &[Arc<Keyframe>], token: &ReadOnlyToken) -> Option<Self> {
        if keyframes.is_empty() {
            return None;
        }

        let animated_properties = get_animated_properties(&keyframes[0], token);
